...
/// Persist the active cabinet hash to `standalone.json`.
fn save_persisted_hash(hash: &str) {
    if let Some(path) = standalone_config_path() {
        if let Some(parent) = path.parent() {
            let _ = std::fs::create_dir_all(parent);
        }
        let json = serde_json::json!({ "cab_active_hash": hash });
                                ...
                                if let Ok(config) = device.default_input_config() {
                                    ...
                                    let stream_res = match config.sample_format() {
                                        cpal::SampleFormat::F32 => {
                                            ...
                                            // Helper para carregar WAV f32 ou i16 (usado apenas pelo Pré-EQ)
                                        ...
                                        cpal::SampleFormat::I16 => {
                                            ...
                                            device.build_input_stream(
                                                &strict_config,
                                                move |data: &[i16], _: &_| {
                                                    let snap = st_clone2.try_lock().map(|g| g.audio()).unwrap_or_else(|_| AudioSnapshot::default());
                            ...
                            if let Some(device) = devs.into_iter().find(|d| d.name().unwrap_or_default() == raw_name) {
                                if let Ok(config) = device.default_output_config() {
                                    ...
                                    match stream_res {
                                        ...
                                        Err(e) => { err_msg = Some(format!("Erro out: {:?}", e)); }
                                    }
                                }
                            } else {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("top_panel").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                let current_bypass = self.standalone_state.lock().map(|g| g.bypass).unwrap_or(false);
        ...
        render_shared_panels(
            ...
            |ui| {
                // Build a runtime for `hash`/`bytes` and hand it to the audio thread.
                let build_and_publish = |bytes: &[u8]| {
