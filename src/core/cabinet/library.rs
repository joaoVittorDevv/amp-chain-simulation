use super::types::{CabinetError, CabinetRuntime, IrMeta};
...
impl CabinetLibrary {
    ...
    pub fn import_ir(&self, raw_bytes: &[u8], filename: &str) -> Result<IrMeta, CabinetError> {
        ...
        // Full-decode validation: reject truncated/corrupt files before storing.
        CabinetRuntime::validate_decodable(raw_bytes)?;
...
mod tests {
    ...
    /// Synthesize an in-memory WAV with the given channel count and frame count.
    fn make_wav(channels: u16, frames: usize) -> Vec<u8> {
    ...
    #[test]
    fn rejects_non_mono_stereo() {
        let lib = mem_lib();
        let three = make_wav(3, 100);
        assert!(lib.import_ir(&three, "3ch.wav").is_err());
    ...
    fn rejects_truncated_wav() {
        let lib = mem_lib();
        let mut good = make_wav(2, 100);
        good.truncate(good.len() - 20); // chop off the tail of the data chunk
        assert!(lib.import_ir(&good, "trunc.wav").is_err());
        assert!(lib.list_irs().unwrap().is_empty());
    }
